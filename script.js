...
function enquire(name){let msg=encodeURIComponent(`Hello Jagdamba Optical, I am interested in the ${name} frame. Please share availability and details.`);window.open(`https://wa.me/91XXXXXXXXXX?text=${msg}`,"_blank")}
function sendWhatsApp(e){e.preventDefault();let n=document.getElementById("name").value,p=document.getElementById("phone").value,m=document.getElementById("message").value;let msg=encodeURIComponent(`Hello Jagdamba Optical,%0AName: ${n}%0APhone: ${p}%0ARequirement: ${m}`);window.open(`https://wa.me/91XXXXXXXXXX?text=${msg}`,"_blank")}
